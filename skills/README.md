...
- [model-tiering](model-tiering/SKILL.md) — Agent tool でサブエージェントに委譲するとき、タスクの性質（判断・統合 / 実装 / 単純作業）に応じて `model` パラメータを使い分けるための割り当て表と運用ルール。
...
- [ros2-debug](ros2-debug/SKILL.md) — ROS 2（Humble）システムのデバッグ手順。ノード疎通・QoS 不一致・TF・パラメータ/ライフサイクル・colcon ビルドの順に切り分けるチェックリスト。
